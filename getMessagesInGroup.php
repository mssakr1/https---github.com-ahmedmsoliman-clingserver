...
if (!isset($u_id) || !isset($accceesToken) ||!isset($id_brand) ||!isset($g_id) ||
        empty($u_id) || empty($accceesToken) || empty($id_brand)|| empty($g_id)) {
...
if (!($stmt = $mysqli->prepare("SELECT * FROM messages_user_brands where  id_brand=? and g_id=? and sender = 2 and id > ?;"))) {
$arr = array ('status'=>-1,'message'=>'error in DB connection');
    echo json_encode($arr);
    return;
...
if (!$stmt->bind_param('iii',$id_brand,$g_id,$last_mesasge_id)) {
