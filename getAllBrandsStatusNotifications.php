...
if (!isset($u_id) || !isset($accceesToken) ||!isset($query) ||
...
if ($result_id == -2) {
    ...
    $arr = array('status' => -1, 'message' => 'error in DB connection5 ');
    echo json_encode($arr);
    return;
}
...
on b.id=x.id_brand  where name like ? ;"))) {
...
$query = '%' . $query . '%';
if (!$stmt->bind_param('is', $u_id,$query)) {

$arr = array ('status'=>-1,'message'=>'error in DB connection3');
    echo json_encode($arr);
...
if (!$stmt->execute()) {
$arr = array ('status'=>-1,'message'=>'error in DB connection2');
    echo json_encode($arr);
    return;
...
$arr = array ('status'=>-1,'message'=>'error in DB connection1');
    echo json_encode($arr);
    return;
