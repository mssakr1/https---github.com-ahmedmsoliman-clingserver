...
if (!isset ($u_id)||!isset ($accceesToken)||!isset ($br_id)||!isset ($group_id)||
    empty($u_id)||empty($accceesToken) ||empty($br_id)  ||empty($group_id) ) {
...
if (!($stmt = $mysqli->prepare("delete from  `group` where id=?"))) {
$arr = array ('status'=>-1,'message'=>'error in DB connection');
    echo json_encode($arr);
    return;
}
